...
/**
 * Created by YuanZhiQiang on 2019/03/08 16:02.
 * 懒加载Fragment
 */
abstract class BaseLazyLoadFragment : BaseFragment(){
    ...
    private fun isParentInvisible(): Boolean {
        ...
        return if (parentFragment is BaseLazyLoadFragment) {
        ...
        if (!fragments.isEmpty()) {
            for (child in fragments) {
                if (child is BaseLazyLoadFragment && !child.isHidden() && child.getUserVisibleHint()) {
                    (child as BaseLazyLoadFragment).dispatchUserVisibleHint(visible)
                }
            }
        }
    }
